...
export class UIManager {
	...
	private setupInstallButtonEvents(): void {
		...
		this.installButton.on('pointerdown', () => {
			...
			try {
				const url = (window as any).MRAID_URL || (window as any).clickTag || (window as any).CLICKTAG || undefined;
				if ((window as any).ExitApi && typeof (window as any).ExitApi.exit === 'function') {
					(url !== undefined) ? (window as any).ExitApi.exit(url) : (window as any).ExitApi.exit();
				}
			} catch (error) {}
